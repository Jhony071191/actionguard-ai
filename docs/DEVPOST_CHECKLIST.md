# Devpost submission checklist

## Ready in the repository

- [x] Original clean-room code and synthetic dataset created after the competition opened
- [x] Functional React, TypeScript, and Vite product demo
- [x] Deterministic ALLOW / REVIEW / DENY engine
- [x] Human approval, idempotency, tenant isolation, masking, and hash-chain evidence
- [x] Policy Studio with explicit deterministic fallback
- [x] English README, MIT license, architecture, API, security, data model, tests, and Xano build story
- [x] Reproducible three-scenario dataset and commands
- [x] Versioned XanoScript backend and browser adapter
- [x] Official XanoScript validation for all 15 constructs
- [x] Devpost project-page draft and three-minute demo script
- [x] Public Netlify deployment and desktop browser smoke test
- [x] Public GitHub repository with verified project history
- [x] Four-minute narration, shot list, and editing plan prepared with a safe 3:55 export target

## Must be completed before final submission

- [x] Create/push the public `Jhony071191/actionguard-ai` GitHub repository
- [x] Connect and verify the existing Xano instance as the central backend
- [x] Re-run the contract and scenario suite against deployed Xano
- [x] Deploy and test the public local-fallback URL
- [x] Upload and retest the Xano-enabled production build on the public URL
- [ ] Record a concise demo video showing all three decisions and approval execution
- [x] Add the project story, technology list, and repository/demo links to Devpost
- [x] Create the nine final production screenshots
- [ ] Add the screenshots and the real 2–4 minute demo video to Devpost
- [ ] Select the Xano sponsor prize; Overall consideration is automatic
- [ ] Confirm the final rules and submit before the official deadline

The Devpost draft exists and its public preview was checked. The public Xano frontend, full desktop journey, nine screenshots, narration, and editing plan are ready. Do not submit yet: the real video, media upload, Xano sponsor selection, mobile sanity check, and final review remain open.
