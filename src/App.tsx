  ...
  return <div className="shell">
    ...
    <main id="top">
      ...
      {view === 'command' && <>
        ...
        {selected?<section className={`result-card result-${tone[selected.decision]}`} aria-live="polite"><div><p className="eyebrow">LATEST DECISION</p><div className={`pill ${tone[selected.decision]}`}>{selected.decision}</div></div><div><h3>{selected.action}</h3><p>{selected.explanation}</p><small>Status {selected.status} · Risk {selected.riskScore} · {chainValid===undefined?'Checking evidence':chainValid?'Audit chain verified':'Audit chain invalid'}</small></div><button className="secondary" onClick={()=>setView('evidence')}>Inspect evidence →</button></section>:null}
      ...
      {view === 'evidence' && <section className="evidence-layout"><div className="panel"><p className="eyebrow">ACTION INSPECTOR</p><h2>Decision evidence</h2>{actions.length === 0 ? <Empty text="Run a scenario to produce auditable evidence."/> : <div className="action-list">{actions.map(action => <button key={action.id} onClick={() => void inspect(action)} className={selected?.id === action.id ? 'selected' : ''}><span className={`pill ${tone[action.decision]}`}>{action.decision}</span><div><strong>{action.action}</strong><small>{action.createdAt}</small></div></button>)}</div>}</div><div className="panel code"><div className="chain-head"><p className="eyebrow">HASH CHAIN</p>{selected && <span className={chainValid===undefined?'':chainValid?'valid':'invalid'}>{chainValid===undefined?'Checking…':chainValid?'✓ Verified':'✕ Invalid'}</span>}</div>{selected ? <><h3>{selected.action}</h3><p>{selected.explanation}</p><pre>{JSON.stringify(selected.maskedPayload, null, 2)}</pre><ol className="timeline">{events.map(event => <li key={event.id}><strong>{event.type}</strong><small>#{event.sequence} · {event.eventHash.slice(0,12)}…</small></li>)}</ol></> : <Empty text="Select an evaluated action."/>}</div></section>}
