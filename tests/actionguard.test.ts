...
describe('safety properties', () => {
  ...
  it('degrades safely when AI is unavailable', async () => { const draft=await new ActionGuardService().draftPolicy('Review payments over $1,000',false); expect(draft.generatedBy).toBe('deterministic-fallback'); expect(draft.warnings).toHaveLength(1); });
