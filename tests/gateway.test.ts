...
import { XanoGateway } from '../src/services/gateway';
...
describe('Xano browser adapter', () => {
  ...
  it('fails closed on an unsuccessful response',async()=>{const fetcher=vi.fn(async()=>new Response('{}',{status:403}));const gateway=new XanoGateway('https://example.xano.io/api:actionguard-v1','test-token',fetcher as unknown as typeof fetch);await expect(gateway.listActions('org-demo')).rejects.toThrow('Xano request failed (403)');});
});
