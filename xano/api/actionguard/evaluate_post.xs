query "actions/evaluate" verb=POST {
  ...
  stack {
    db.query "action" {
      where = $db.action.organization_id == $auth.organization_id && $db.action.idempotency_key == $input.idempotency_key
      return = { type: "single" }
    } as $existing
    conditional {
      if ($existing != null) {
    ...
    var $execution { value = $evaluation.decision == "ALLOW" ? {execution_id: $input.idempotency_key|sha256, executed_at: now, effect: "Simulated " ~ $input.action_name ~ " on " ~ $input.resource} : null }
    db.transaction {
      stack {
        db.add "action" {
          data = {
        ...
        function.run "append_audit_event" {
          input = {organization_id: $auth.organization_id, action_id: $created.id, actor_ref: $auth.id|to_text, event_type: "EVALUATED", event_data: {decision: $created.decision, risk_score: $created.risk_score}}
        } as $evaluated_event
        conditional {
          if ($evaluation.decision == "ALLOW") {
            function.run "append_audit_event" {
              input = {organization_id: $auth.organization_id, action_id: $created.id, actor_ref: "system", event_type: "EXECUTED", event_data: $execution}
            } as $executed_event
          }
        }
      }
